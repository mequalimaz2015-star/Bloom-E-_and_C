...
<head>
    ...
    <style>
        :root {
            ...
            --accent: #9a6852;
            --bg-dark: #2a1f18;
            /* Rich Dark Brown instead of Black */
            --text-light: #fefcf9;
            --bg-section: rgba(42, 31, 24, 0.1);
        }
        ...
        * {
        ...
        .site-background {
            ...
            height: 100%;
            z-index: -10;
            background: linear-gradient(to bottom, #2a1f18, #1c1510);
        }
...
<body>
    ...
    <nav>
        ...
        <div class="nav-links">
            <a href="#home">Home</a>
            <div class="dropdown">
                <a href="#about">About Us <i class="fa-solid fa-chevron-down" style="font-size: 10px;"></i></a>
                <div class="dropdown-content">
                    <a href="#ceo-message">Message from CEO</a>
            ...
            <a href="#menu">Menu</a>
            <div class="dropdown">
                <a href="#services">Our Services <i class="fa-solid fa-chevron-down" style="font-size: 10px;"></i></a>
                <div class="dropdown-content">
                    <a href="#services" onclick="filterServices('all')">All Services</a>
                    <?php if (!empty($service_categories)): ?>
                        <?php foreach ($service_categories as $cat): ?>
                            <a href="#services" onclick="filterServices('<?= htmlspecialchars($cat) ?>')">
                                <?= htmlspecialchars($cat) ?>
                            </a>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <!-- Fallback if database categories are empty -->
                        <a href="#services" onclick="filterServices('Food Delivery')">Food Delivery</a>
                        <a href="#services" onclick="filterServices('Catering Service')">Catering Service</a>
                        <a href="#services" onclick="filterServices('Wedding Events')">Wedding Events</a>
                        <a href="#services" onclick="filterServices('Birthday Parties')">Birthday Parties</a>
                        <a href="#services" onclick="filterServices('Corporate Events')">Corporate Events</a>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="about-content" style="display: flex; align-items: center; gap: 80px; flex-wrap: wrap;">
            <div class="ceo-image-wrapper" style="flex: 1; position: relative; min-width: 300px;">
                ...
                <img src="<?= htmlspecialchars(($company['ceo_image'] ?? '') ?: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?q=80&w=1974&auto=format&fit=crop') ?>"
                    alt="CEO Image"
                    style="width: 100%; border-radius: 20px; position: relative; z-index: 2; box-shadow: 0 30px 60px rgba(0,0,0,0.5); display: block;">
            </div>

            <div class="ceo-text" style="flex: 1.2; min-width: 300px;">
                <h2
                    style="font-family:'Playfair Display', serif; font-size: 2.8rem; color: var(--primary); margin-bottom: 5px;">
                    Message from <?= htmlspecialchars(($company['ceo_name'] ?? '') ?: 'The CEO') ?></h2>
                <h4
                    style="font-size: 1.2rem; color: rgba(255,255,255,0.6); margin-bottom: 30px; font-weight: 400; font-style: italic;">
                ...
                <p style="font-size: 1.15rem; line-height: 1.8; color: rgba(255,255,255,0.85); margin-bottom: 30px;">
                    <?= nl2br(htmlspecialchars(($company['ceo_message'] ?? '') ?: 'Welcome to Bloom Africa. We are dedicated to providing the finest dining experience...')) ?>
                </p>

                <div style="display: flex; align-items: center; gap: 20px;">
                    <div style="width: 60px; height: 1px; background: var(--primary);"></div>
                    <span style="font-family: 'Playfair Display', serif; font-size: 1.4rem; color: var(--primary);">With
    ...
    <!-- Our History Section -->
    <section id="history" style="padding: 100px 5%; background: #0f0b09; border-top: 1px solid rgba(223,177,128,0.05);">
    ...
    <section id="team"
        ...
        <div style="text-align: center; margin-bottom: 60px;">
            <h2 style="font-family:'Playfair Display', serif; font-size: 3rem; color: var(--primary);">The Minds Behind
                the Magic</h2>
            <p style="color: rgba(255,255,255,0.6); margin-top: 10px;">Our dedicated team of world-class chefs and
                hospitality experts.</p>
        </div>
        ...
        <div class="gallery">
            <div class="gallery-grid" id="galleryGrid">
                <?php foreach ($gallery_items as $item): ?>
                    <div class="gallery-item" data-category="<?= htmlspecialchars($item['category']) ?>">
                        ...
                        <div class="gallery-overlay">
                            <h4><?= htmlspecialchars($item['title'] ?: $item['category']) ?></h4>
                ...
                <?php if (empty($gallery_items)): ?>
                    ...
                    <div class="gallery-item" data-category="Ambience">
    ...
    <script>
        ...
        function registerChat() {
            ...
            fetch('chat_handler.php', {
                ...
                body: JSON.stringify({ register: true, name, email, phone })
            })
                .then(res => res.json())
                .then(data => {
                    if (data.success) {
